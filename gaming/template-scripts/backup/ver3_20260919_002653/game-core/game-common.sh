...
_install_vcrun2019() {
  # Deterministisch: eerst bestaande runtime valideren, dan zo nodig auto-
  # helen met de x86_64-builtin van de gepinde runner, en winetricks pas als
  # tertiair vangnet. Zo blijft de uitkomst per pc gelijk (geen host-afhan-
  # kelijke winetricks/download meer die een ARM64-dll kan plaatsen).
  local sys32="$PREFIX_PATH/drive_c/windows/system32"
  local dll target src
  # touched: er is een builtin geplaatst. needs_winetricks: iets bleef
  # ongeldig ná de heal-poging (los van elkaar houden voorkomt dat een
  # reeds-geldige dll alsnog winetricks triggert).
  local touched=0
  local needs_winetricks=0

  for dll in msvcp140.dll msvcp140_1.dll; do
    target="$sys32/$dll"
    if _dll_is_x86_64 "$target"; then
      continue
    fi
    if [ -f "$target" ]; then
      _log "$dll is geen x86_64-PE — wordt vervangen (auto-heal)."
    else
      _log "$dll ontbreekt in system32 — wordt aangevuld."
    fi
    if src="$(_proton_builtin_dll "$dll")"; then
      if cp -f "$src" "$target" 2>/dev/null; then
        chmod u+w "$target" 2>/dev/null || true
        _log "Geplaatst: $dll (x86_64 builtin)."
        touched=1
      else
        _log "Kopiëren van $dll mislukt (permissies?)."
        needs_winetricks=1
      fi
    else
      _log "Geen x86_64-builtin voor $dll gevonden in de runner."
      needs_winetricks=1
    fi
  done

  # Eindvalidatie: blijft er een DLL ongeldig, dan is winetricks het vangnet.
  for dll in msvcp140.dll msvcp140_1.dll; do
    if ! _dll_is_x86_64 "$sys32/$dll"; then
      _log "VC-runtime nog incompleet ($dll) na auto-heal."
      needs_winetricks=1
    fi
  done

  if [ "$needs_winetricks" = "1" ]; then
    if command -v winetricks >/dev/null 2>&1; then
      _log "VC++ 2019 alsnog via winetricks (vangnet)..."
      WINEPREFIX="$PREFIX_PATH" winetricks -q vcrun2019 || true
    else
      _log "winetricks niet gevonden; VC-runtime kan incompleet blijven."
    fi
  elif [ "$touched" = "1" ]; then
    _log "VC++ 2019-runtime herstelt via runner-builtins (geen winetricks nodig)."
  else
    _log "VC++ 2019-runtime was al aanwezig en correct (x86_64); niets te doen."
  fi
}
